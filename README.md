...
**Informe Google Docs + correo (dash):** en el **mismo** Google Sheet donde tienes las hojas y funciones por fila, abre **Extensiones → Apps Script** y pega ahí **`st/google-apps-script-informe.gs`** (junto a tus `generarInformesDocs…`); define **`ST_SECRET`**, **Implementar → Aplicación web** y pega la URL `/exec` en el dash (**Herramientas → Config. informe Docs** o constantes en `dash-app.js`). No hace falta un proyecto Apps Script aparte si prefieres todo ligado al libro. Los botones **Generar informe** / **Enviar correo** usan `doPost`; las plantillas y carpetas son las mismas que en Sheets.
